...
class GameView(context: Context, private var trolleybus: Trolleybus) :
    ...
    // Raycasting
    private val FOV = PI / 3.0         // 60 градусов
    private val MAX_DEPTH = 20.0
    private val NUM_RAYS get() = screenW
    ...
    init {
        ...
        paintOverlay.color = Color.argb(180, 0, 0, 0)
    }
    ...
    private fun setupButtons() {
        ...
        btnBrake = RectF(gap + bw + gap,  by, gap*2 + bw*2,   by + bh)
    ...
    private fun drawRaycast(canvas: Canvas) {
        ...
        for (ray in 0 until NUM_RAYS) {
            val rayAngle = trolleybus.angle - (FOV / 2).toFloat() +
                           (ray.toDouble() / NUM_RAYS * FOV).toFloat()
            ...
            if (hit) {
                ...
                dist *= cos(rayAngle - trolleybus.angle)
                ...
                // Затемнение по дистанции и стороне
    ...
    private fun drawWires(canvas: Canvas) {
        // Провода КС — две горизонтальные линии в верхней части экрана
    ...
    private fun drawHUD(canvas: Canvas) {
        // Табло (верхний левый угол)
        drawBoard(canvas)

        // Скорость
        val kmh = (trolleybus.speed / maxSpeed * 60).toInt()
        paintHUD.textSize = 36f
        paintHUD.color = Color.WHITE
        ...
        val powerText = if (trolleybus.powerState == PowerState.ON) "⚡ ТОК" else "〇 НАКАТ"
    ...
    private fun drawBoard(canvas: Canvas) {
        ...
        // Номер троллейбуса (маленький)
        paintBoard.textSize = 22f
        paintBoard.color = Color.GRAY
        canvas.drawText("БКМ 321  №${trolleybus.number}", screenW * 0.57f, screenH * 0.21f, paintBoard)
    }

    private fun drawSwitchWarning(canvas: Canvas, sw: Switch) {
        val msg = if (sw.requiresPower) "⚡ РАЗГАНЯЙЦЕСЯ!" else "〇 НАКАТАМ!"
        val sub = sw.desc
        ...
        canvas.drawText(sub, screenW * 0.5f, screenH * 0.70f, paintWarning)
    ...
    private fun drawButtons(canvas: Canvas) {
        ...
        drawBtn(canvas, btnDoors, if (trolleybus.doorsOpen) "ЗАЧЫН" else "ДЗВЕРЫ",
